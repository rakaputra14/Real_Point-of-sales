        ...
        <form action="{{ route('product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="product_name" id="product_name" class="form-control"
                    placeholder="Enter product name" value="{{ old('product_name', $product->product_name) }}" autofocus>
                @error('product_name')
                    <div class="text-danger small">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                @if($product->product_photo)
                    <div class="mt-2">
                        <img src="{{ asset('storage/' . $product->product_photo) }}" alt="Product Photo"
            ...
            <div class="mb-3">
                ...
                <input type="number" name="product_qty" id="product_qty" class="form-control"
                    placeholder="Enter product price" value="{{ old('product_qty', $product->product_qty) }}">
                @error('product_qty')
                    <div class="text-danger small">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="d-flex justify-content-end">
                ...
                <button type="submit" class="btn btn-primary"
                    style="padding: 0.5rem 1rem; height: 2.5rem; line-height: 1.5rem;">Update</button>
            </div>
        </form>
    </section>
